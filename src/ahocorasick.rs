...
/// but its memory usage is quite small. Its search speed (not listed) is also
/// often faster than a noncontiguous NFA, but a little slower than a DFA.
/// Indeed, when [`AhoCorasickKind::Auto`] is used (which is the default), a
/// contiguous NFA is used in most cases.
///
/// # Cloning
///
...
pub struct AhoCorasick {
    ...
    ///
    /// So why record the start kind here? Well, consider what happens when
    /// 'AhoCorasickKind::Auto' and 'StartKind::Unanchored' are used (both are
    /// the default). It *might* result in using a DFA or it might pick an
    /// NFA. If it picks an NFA, the caller would then be able to run anchored
    /// searches, even though the caller only asked for support for unanchored
    /// searches. Maybe that's fine, but what if the DFA was chosen instead?
    /// Oops, the caller would get an error.
    ///
...
impl AhoCorasick {
    ...
    ///
    /// let ac = AhoCorasick::builder()
    ///     .kind(AhoCorasickKind::Auto) // default
...
/// to always use a [`AhoCorasickKind::DFA`] if search speed is critical
/// and memory usage isn't a concern. Otherwise, [`AhoCorasickKind::Auto`]
/// will probably make the right choice for you. Beware that if you use
...
impl AhoCorasickBuilder {
        ...
        P: AsRef<[u8]>,
    {
        ...
        let aut: Arc<dyn AcAutomaton> = match self.kind {
            AhoCorasickKind::Auto => {
                debug!(
        ...
        Ok(AhoCorasick { aut, start_kind: self.start_kind })
    }
    ...
    /// that first.
    fn build_auto(&self, nfa: noncontiguous::NFA) -> Arc<dyn AcAutomaton> {
    ...
    /// * [`AhoCorasickKind::Auto`] (the default) instructs the searcher to
    /// choose the "best" Aho-Corasick implementation. This choice is typically
    /// based primarily on the number of patterns.
    ///
    ...
    ///
    /// In general, you should probably stick to the default unless you
    /// have some kind of specific reason to use a specific Aho-Corasick
    /// implementation. For example, you might choose `AhoCorasickKind::DFA` if
    /// you don't care about memory usage and want the fastest possible search
    /// times.
    ///
    /// Note that setting this guarantees that the searcher returned uses
    /// the chosen implementation. If that implementation could not be
    /// constructed, then an error will be returned. In contrast, when
    /// `AhoCorasickKind::Auto` is used, it is possible for it to attempt to
    /// construct, for example, a contiguous NFA and have it fail. In which
    /// case, it will fall back to using a noncontiguous NFA.
    pub fn kind(&mut self, kind: AhoCorasickKind) -> &mut AhoCorasickBuilder {
        self.kind = kind;
        self
    }
...
pub enum AhoCorasickKind {
    /// The default. The Aho-Corasick builder will choose the best
    /// implementation to use. Unless you have a specific reason otherwise,
...
impl Default for AhoCorasickKind {
    fn default() -> AhoCorasickKind {
        AhoCorasickKind::Auto
    }
}
