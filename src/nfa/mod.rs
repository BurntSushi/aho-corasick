...
uses more memory, but permits computing transitions more quickly. By only
making the most active states dense (the states near the starting state), a
contiguous NFA better balances memory usage with search speed. The single
contiguous allocation uses less overhead per state and enables compression
tricks where most states only use 8 bytes of heap memory.
